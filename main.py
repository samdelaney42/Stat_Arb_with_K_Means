from model.stat_arb_model import StatArbBuilder, StatArb
from model.stat_arb_clustering import clusters
...
while user != "stop":
	...
	while in_cluster != 0:
		...
		valid_pair = StatArbBuilder(name_1, name_2, start, end).build()

		if valid_pair == True:
			model = StatArb(name_1, name_2, start, end)
			model.backtest_pair()
		else:
