...
class Clusters(object):
	...
	def generate_std_data(self):
		...
		self.data_std = pd.DataFrame(self.data_std).dropna()
	...
	def generate_pca(self):
		...
		'''
		# pca model
		self.pca = PCA(n_components = len(self.data_std.columns))
	...
	def generate_clusters(self):
		...
		if os.path.isfile('data/clusters/labled_stock_clusters_{}.csv'.format(date.today())):
		    print("yeet")
		    self.two_or_more = pd.read_csv('data/clusters/labled_stock_clusters_{}.csv'.format(date.today()))
		    self.two_or_more = self.two_or_more.set_index("Symbol")
		    self.K = len(self.two_or_more["Clusters"].unique())
		    print(self.K)
			...
			self.two_or_more.to_csv('data/clusters/labled_stock_clusters_{}.csv'.format(date.today()))
	...
	def generate_pairs(self, which_cluster, p):
		'''
		parameters
		----------
			self: object 
